...
bool BaseSprite::isContainPoint(cocos2d::Touch *touch, cocos2d::Event *event)
{
    auto target = static_cast<BaseSprite*>(event->getCurrentTarget());
    if(target->getTouchEnabled()==false)
    {
        return false;
    }
...
void BaseSprite::__initEventListener()
{
    ...
    eventListener->onTouchBegan = [&](Touch *touch,Event *event)->bool{
        ...
        auto target = static_cast<BaseSprite*>(event->getCurrentTarget());
        if (this->isContainPoint(touch, event)) {
    ...
    eventListener->onTouchEnded = [&](Touch *touch,Event *event)->void{
        ...
        auto target = static_cast<BaseSprite*>(event->getCurrentTarget());
        if(this->isContainPoint(touch, event)&&target->touchEndedHandler&&GameManager::getInstance()->getIsScroll()==false)
        {
    ...
    eventListener->onTouchMoved = [&](Touch *touch,Event *event)->void{
        ...
        if(this->isContainPoint(touch, event)&&target->touchMovedHandler)
        {
